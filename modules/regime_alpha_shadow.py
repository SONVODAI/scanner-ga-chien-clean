...
def build_shadow_with_recall(
    ...
    *,
    ...
    shadow_df["BaselineRank"] = (
        shadow_df["BaselineScore"]
        .rank(method="first", ascending=False)
        .astype(int)
    )
    shadow_df["ShadowExperienceRank"] = (
        shadow_df["ShadowExperienceScore"]
        .rank(method="first", ascending=False)
        .astype(int)
    )
    shadow_df["RankDelta"] = (
        shadow_df["BaselineRank"] - shadow_df["ShadowExperienceRank"]
    )
    shadow_df["ShadowMovement"] = shadow_df.apply(
        lambda r: _movement(int(r["BaselineRank"]), int(r["ShadowExperienceRank"])),
        axis=1,
    )
    return shadow_df.sort_values(
        ["BaselineRank", "symbol"], kind="stable"
