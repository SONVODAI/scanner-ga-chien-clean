"""
Phase 3J.3 — Durable first-experiment execution persistence.
"""

from __future__ import annotations

import json
import os
from pathlib import Path
from typing import Any, Dict, Optional

from modules.edge_research.opr_bridge.evidence_synthesis_records import utc_now_iso
from modules.edge_research.opr_bridge.first_experiment_execution_records import (
    ExecutionBindingAudit,
    FirstExperimentExecutionEnvelope,
)
from modules.edge_research.opr_bridge.first_experiment_records import (
    FirstExperimentCandidateRecord,
    InitialExperimentObjectiveRecord,
    InitialExperimentPackage,
)
from modules.edge_research.storage import resolve_data_dir

PERSISTENCE_VERSION = "first_experiment_execution_persistence_v1_3j3"
EXECUTIONS_DIR = "first_experiment_executions"
EXECUTION_INDEX_FILE = "first_experiment_execution_index.json"


def executions_dir(data_dir: Optional[Path] = None) -> Path:
    root = resolve_data_dir(data_dir)
    path = root / EXECUTIONS_DIR
    path.mkdir(parents=True, exist_ok=True)
    return path


def execution_index_path(data_dir: Optional[Path] = None) -> Path:
    return resolve_data_dir(data_dir) / EXECUTION_INDEX_FILE


def _atomic_write(path: Path, payload: str) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + ".tmp")
    tmp.write_text(payload, encoding="utf-8")
    os.replace(tmp, path)


def envelope_to_dict(envelope: FirstExperimentExecutionEnvelope) -> Dict[str, Any]:
    d = envelope.to_dict()
    d["persistence_version"] = PERSISTENCE_VERSION
    return d


def envelope_from_dict(payload: Dict[str, Any]) -> FirstExperimentExecutionEnvelope:
    audit_d = payload["binding_audit"]
    audit = ExecutionBindingAudit(
        scientific_spec_hash=audit_d["scientific_spec_hash"],
        execution_spec_hash=audit_d["execution_spec_hash"],
        scientific_action_core_hash=audit_d["scientific_action_core_hash"],
        population_spec=dict(audit_d["population_spec"]),
        outcome_spec=dict(audit_d["outcome_spec"]),
        observation_horizon=int(audit_d["observation_horizon"]),
        tool_name=audit_d["tool_name"],
        tool_version=audit_d["tool_version"],
        inputs=dict(audit_d["inputs"]),
        binding_notes=tuple(audit_d.get("binding_notes") or ()),
    )
    return FirstExperimentExecutionEnvelope(
        execution_id=payload["execution_id"],
        record_version=payload["record_version"],
        package_id=payload["package_id"],
        package_hash=payload["package_hash"],
        proposition_id=payload["proposition_id"],
        proposition_hash=payload["proposition_hash"],
        session_id=payload["session_id"],
        selected_candidate_id=payload["selected_candidate_id"],
        scientific_action_core_hash=payload["scientific_action_core_hash"],
        experiment_content_hash=payload["experiment_content_hash"],
        execution_identity_hash=payload["execution_identity_hash"],
        binding_audit=audit,
        tool_result=dict(payload["tool_result"]),
        tool_result_hash=payload["tool_result_hash"],
        raw_quintile_metrics=payload.get("raw_quintile_metrics"),
        panel_provenance_hash=payload["panel_provenance_hash"],
        execution_outcome=payload["execution_outcome"],
        tool_status=payload["tool_status"],
        sample_size=int(payload.get("sample_size", 0)),
        warnings=tuple(payload.get("warnings") or ()),
        errors=tuple(payload.get("errors") or ()),
        executor_version=payload["executor_version"],
        created_at=payload["created_at"],
        envelope_hash=payload["envelope_hash"],
    )


def package_from_dict(payload: Dict[str, Any]) -> InitialExperimentPackage:
    objectives = tuple(
        InitialExperimentObjectiveRecord(
            objective_id=o["objective_id"],
            record_version=o["record_version"],
            proposition_id=o["proposition_id"],
            proposition_hash=o["proposition_hash"],
            target_uncertainty=o["target_uncertainty"],
            scientific_vulnerability=o["scientific_vulnerability"],
            why_first=o["why_first"],
            outcome_branches=dict(o["outcome_branches"]),
            forbidden_rescue_mutations=tuple(o.get("forbidden_rescue_mutations") or ()),
            provenance=dict(o.get("provenance") or {}),
            directness_rank=int(o["directness_rank"]),
            created_at=o["created_at"],
            objective_hash=o["objective_hash"],
        )
        for o in payload.get("objectives") or []
    )

    def _cand(c: Dict[str, Any]) -> FirstExperimentCandidateRecord:
        return FirstExperimentCandidateRecord(
            candidate_id=c["candidate_id"],
            record_version=c["record_version"],
            proposition_id=c["proposition_id"],
            proposition_hash=c["proposition_hash"],
            objective_id=c["objective_id"],
            scientific_action_core_hash=c["scientific_action_core_hash"],
            scientific_identity=dict(c["scientific_identity"]),
            primary_classification=c["primary_classification"],
            secondary_classifications=tuple(c.get("secondary_classifications") or ()),
            classification_rationale=c["classification_rationale"],
            falsification_capable=bool(c["falsification_capable"]),
            confirmatory_only=bool(c["confirmatory_only"]),
            birth_evidence_overlap_fraction=float(c["birth_evidence_overlap_fraction"]),
            independence_profile=dict(c.get("independence_profile") or {}),
            directness_rank=int(c["directness_rank"]),
            epistemic_alteration_potential=c["epistemic_alteration_potential"],
            rescue_risk_status=c["rescue_risk_status"],
            executability_status=c["executability_status"],
            executability_detail=c["executability_detail"],
            experiment_spec=c.get("experiment_spec"),
            representation_envelope=dict(c.get("representation_envelope") or {}),
            created_at=c["created_at"],
            record_hash=c["record_hash"],
        )

    return InitialExperimentPackage(
        package_id=payload["package_id"],
        record_version=payload["record_version"],
        proposition_id=payload["proposition_id"],
        proposition_hash=payload["proposition_hash"],
        generator_version=payload["generator_version"],
        selector_version=payload["selector_version"],
        objectives=objectives,
        candidates_considered=tuple(_cand(c) for c in payload.get("candidates_considered") or []),
        deduplicated_candidates=tuple(_cand(c) for c in payload.get("deduplicated_candidates") or []),
        rejected=tuple(dict(r) for r in payload.get("rejected") or []),
        ranking_trace=tuple(dict(t) for t in payload.get("ranking_trace") or []),
        disposition=payload["disposition"],
        selected_candidate_id=payload.get("selected_candidate_id"),
        selected_experiment_spec=payload.get("selected_experiment_spec"),
        selection_reason=payload.get("selection_reason", ""),
        human_choice_material=bool(payload.get("human_choice_material", False)),
        human_choice_reason=payload.get("human_choice_reason", ""),
        execution_status=payload.get("execution_status", "NOT_EXECUTED"),
        created_at=payload.get("created_at", utc_now_iso()),
        package_hash=payload["package_hash"],
    )


def load_execution_index(data_dir: Optional[Path] = None) -> Dict[str, str]:
    path = execution_index_path(data_dir)
    if not path.exists():
        return {}
    return json.loads(path.read_text(encoding="utf-8"))


def save_execution_index(index: Dict[str, str], data_dir: Optional[Path] = None) -> Path:
    path = execution_index_path(data_dir)
    _atomic_write(path, json.dumps(index, indent=2, sort_keys=True))
    return path


def execution_envelope_path(execution_id: str, data_dir: Optional[Path] = None) -> Path:
    safe = execution_id.replace("/", "_")
    return executions_dir(data_dir) / f"{safe}.json"


def persist_execution_envelope(
    envelope: FirstExperimentExecutionEnvelope,
    data_dir: Optional[Path] = None,
) -> Path:
    path = execution_envelope_path(envelope.execution_id, data_dir=data_dir)
    _atomic_write(path, json.dumps(envelope_to_dict(envelope), indent=2, default=str))
    index = load_execution_index(data_dir)
    index[envelope.execution_identity_hash] = envelope.execution_id
    save_execution_index(index, data_dir=data_dir)
    return path


def lookup_execution_by_identity(
    execution_identity_hash: str,
    data_dir: Optional[Path] = None,
) -> Optional[FirstExperimentExecutionEnvelope]:
    index = load_execution_index(data_dir)
    eid = index.get(execution_identity_hash)
    if not eid:
        return None
    path = execution_envelope_path(eid, data_dir=data_dir)
    if not path.exists():
        return None
    return envelope_from_dict(json.loads(path.read_text(encoding="utf-8")))
