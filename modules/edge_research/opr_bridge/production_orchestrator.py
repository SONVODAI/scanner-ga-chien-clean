...
def run_production_opr_cycle(
    ...
    *,
    ...
    if existing:
        ...
        if decide_first_experiment and record.first_experiment_research_decision:
            result.first_experiment_research_decision = record.first_experiment_research_decision
            if STOP_RESEARCH_DECISION_FROZEN not in result.stop_boundaries:
                result.stop_boundaries.append(STOP_RESEARCH_DECISION_FROZEN)
        if design_second_experiment and record.second_experiment_package:
        ...
        if decide_second_experiment and record.second_experiment_research_decision:
    ...
    if execute_first_experiment and detection.proposition_record:
        ...
        if interpret_first_experiment and record.first_experiment_execution and record.frozen_interpretation_contract:
            ...
            if decide_first_experiment and record.first_experiment_interpretation:
                ...
                dx = run_production_first_experiment_research_decision(
                    ...
                    session_id=session_id,
                    package_dict=record.initial_experiment_package or {},
                    interpretation_dict=record.first_experiment_interpretation,
                    data_dir=data_dir,
                )
                if dx.decision and dx.decision.envelope:
                    record.first_experiment_research_decision = dx.decision.envelope.to_dict()
                stop_boundaries.append(STOP_RESEARCH_DECISION_FROZEN)
                write_opr_session(record, data_dir=data_dir)
                result.first_experiment_research_decision = dx.to_dict()

                if design_second_experiment and record.first_experiment_research_decision:
                    ...
                    sx = run_production_second_experiment_design(
                        ...
                        session_id=session_id,
                        package_dict=record.initial_experiment_package or {},
                        execution_dict=record.first_experiment_execution or {},
                        interpretation_dict=record.first_experiment_interpretation or {},
                        decision_dict=record.first_experiment_research_decision,
                        data_dir=data_dir,
                    )
                    if sx.design and sx.design.package:
                        record.second_experiment_package = sx.design.package.to_dict()
                    stop_boundaries.append(STOP_SECOND_EXPERIMENT_DESIGNED)
                    write_opr_session(record, data_dir=data_dir)
                    ...
                    if execute_second_experiment and record.second_experiment_package and record.first_experiment_execution:
                        ...
                        if (
                            ...
                            if (
                                decide_second_experiment
                                and record.second_experiment_interpretation
                                and record.first_experiment_research_decision
                            ):
                                ...
                                dx2 = run_production_second_experiment_research_decision(
                                    detection.proposition_record,
                                    session_id=session_id,
                                    second_interpretation_dict=record.second_experiment_interpretation,
                                    first_decision_dict=record.first_experiment_research_decision,
                                    first_interpretation_dict=record.first_experiment_interpretation,
                                    data_dir=data_dir,
                                )
                                if dx2.decision and dx2.decision.envelope:
