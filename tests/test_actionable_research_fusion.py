...
def _bar(symbol: str, ts: datetime, close: int = 20000, volume: int = 1000) -> CanonicalBar:
    return CanonicalBar(
        symbol=symbol,
        timestamp=ts,
...
def _write_camera(
    ...
    *,
    ...
    volumes = volumes or {}
    for sym in UNIVERSE:
        bars.append(_bar(sym, ts, volume=int(volumes.get(sym, 1_000))))
    if extra_late:
...
def test_1_142_stock_eligible_session(fusion_paths):
    ...
    assert payload["record_count"] == 142
    assert len(payload["records"]) == 142
...
def test_8_missing_camera_feed_is_unknown_not_normal(fusion_paths):
    ...
    assert rec["camera_data_status"] == CAMERA_DATA_FEED_MISSING
    assert rec["money_flow_status"] not in {"MONEY_FLOW_NORMAL", "MONEY_FLOW_WEAK"}
    assert "CAMERA" in (rec["missing_evidence"][0] if rec["missing_evidence"] else "CAMERA_FEED")
...
def test_9_missing_foreign_is_unknown_not_zero(fusion_paths):
    ...
    payload = fuse_session(TRADE_DATE, paths=fusion_paths)
    rec = payload["records"][0]
    assert rec["foreign_flow_status"] == FOREIGN_UNKNOWN
    assert rec["foreign_net_value"] is None
    assert rec["foreign_net_value"] != 0
    assert rec["foreign_data_completeness"] == "UNAVAILABLE"
    assert any("FOREIGN" in x for x in rec["missing_evidence"])
...
def test_11_same_date_replay_no_duplicate_records(fusion_paths):
    ...
    assert second["record_count"] == first["record_count"] == 142
...
def test_12_pit_cutoff_drops_future_bars(fusion_paths):
    ...
    rec = _by_symbol(payload, hot)
    metrics = rec["camera_metrics"]
    assert metrics.get("session_volume") == 1_000
    assert payload["camera"]["look_ahead_bars_dropped"] >= 1
    assert rec["camera_cutoff_timestamp"].startswith("2026-08-14T11:00:00")
