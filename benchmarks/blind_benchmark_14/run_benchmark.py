#!/usr/bin/env python3
"""
Blind Benchmark 14 — Phase 3H.13 Semantic Novelty Rank Reconciliation.

Orchestration and reporting ONLY. Does NOT modify research logic.
Frozen research commit: 48114b4bd (Phase 3H.13).
Uses SAME frozen panel fingerprint as BB01–BB13.
...
BB13_ARTIFACTS = REPO / "benchmarks" / "blind_benchmark_13" / "artifacts"
BB01_ARTIFACTS = REPO / "benchmarks" / "blind_benchmark_01" / "artifacts"
BB12_ARTIFACTS = REPO / "benchmarks" / "blind_benchmark_12" / "artifacts"
BB11_ARTIFACTS = REPO / "benchmarks" / "blind_benchmark_11" / "artifacts"
...
BENCHMARK_ID = "blind_benchmark_14"
BENCHMARK_VERSION = "bb14_v1"
SESSION_ID = "bb14-autonomous-001"
FROZEN_RESEARCH_COMMIT = "48114b4bd"
...
from modules.edge_research.research_proposition_core import RESEARCH_PROPOSITION_CORE_VERSION  # noqa: E402
from modules.edge_research.research_novelty_valuation_bridge import (  # noqa: E402
    RESEARCH_NOVELTY_VALUATION_BRIDGE_VERSION,
)
from modules.edge_research.research_novelty_rank_reconciliation import (  # noqa: E402
...
def build_bb12_freeze_manifest(
    ...
    base.update(
        {
            ...
            "research_novelty_valuation_bridge_version": RESEARCH_NOVELTY_VALUATION_BRIDGE_VERSION,
            "research_novelty_rank_reconciliation_version": RESEARCH_NOVELTY_RANK_RECONCILIATION_VERSION,
            "session_id": SESSION_ID,
            "bb14_primary_question": (
                "Does Phase 3H.13 semantic rank reconciliation make autonomous experiment "
                "selection respect representation vs scientific novelty distinction?"
            ),
            "modification_policy": "NO_RESEARCH_LOGIC_CHANGES_BEFORE_DURING_OR_AFTER_BB14",
        }
    )
    return base
...
def build_run_configuration(manifest: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "phase_versions": {
            ...
            "research_novelty_valuation_bridge": RESEARCH_NOVELTY_VALUATION_BRIDGE_VERSION,
        },
...
def extract_session_trails(graph: Any) -> Dict[str, List[Dict[str, Any]]]:
    return {
        ...
        "research_novelty_gating_audit": list(
            getattr(graph.session, "research_novelty_gating_audit", None) or []
        ),
        "research_rank_reconciliation_audit": list(
            getattr(graph.session, "research_rank_reconciliation_audit", None) or []
        ),
    }
...
def load_bb11_baseline() -> Dict[str, Any]:
    bb11_summary = _load_json(BB11_ARTIFACTS / "02_session_summary.json") or {}
    bb11_stop = _load_json(BB11_ARTIFACTS / "12_stop_continue_quality_classification.json") or {}
    bb11_remaining = _load_json(BB11_ARTIFACTS / "16_remaining_opportunity_audit.json") or {}
    bb11_global = _load_json(BB11_ARTIFACTS / "11_global_allocation_diary.json") or []
    return {
        "available": bool(bb11_summary),
        "session_id": bb11_summary.get("session_id", "bb11-autonomous-001"),
        "experiments_used": bb11_summary.get("experiments_used"),
        "session_status": bb11_summary.get("session_status"),
        "late_mechanical_cycling": bb11_summary.get("late_mechanical_cycling"),
        "bridge_changed_decisions": sum(
            1
            for iv in (_load_json(BB11_ARTIFACTS / "07_information_value_audit.json") or [])
            if iv.get("selection_changed")
        ),
        "exit_valuation_active": bb11_summary.get("exit_valuation_audit_entries", 0) > 0,
        "stop_competed_count": bb11_summary.get("stop_competed_count", 0),
        "selection_changed_by_exit_count": bb11_summary.get("selection_changed_by_exit_count", 0),
        "tool_distribution": bb11_summary.get("tool_distribution", {}),
        "mechanical_cycling_total": bb11_stop.get("continue_quality_counts", {}).get(
            "MECHANICAL_CYCLING", 0
        ),
        "premature_stop_count": bb11_stop.get("stop_quality_counts", {}).get("PREMATURE_STOP", 0),
        "unexplored_frontier_at_stop": bb11_remaining.get("frontier_unexplored_count"),
        "global_allocation_decisions": len(bb11_global) if bb11_global else 0,
        "novelty_gating_active": False,
        "novelty_gating_applied_count": 0,
    }


def build_rank_reconciliation_audit_report(
    rank_audit: List[Dict[str, Any]],
) -> Dict[str, Any]:
    applied = [e for e in rank_audit if e.get("reconciliation_applied")]
    return {
        "research_novelty_rank_reconciliation_version": RESEARCH_NOVELTY_RANK_RECONCILIATION_VERSION,
        "audit_entries": rank_audit,
        "entry_count": len(rank_audit),
        "reconciliation_applied_count": len(applied),
        "total_planner_novelty_delta": sum(float(e.get("planner_novelty_delta") or 0) for e in rank_audit),
        "representation_only_reconciliations": sum(
            1
            for e in applied
            if e.get("valuation_class") == "REPRESENTATION_NOVELTY_ONLY"
        ),
    }


def load_bb13_baseline() -> Dict[str, Any]:
    bb13_summary = _load_json(BB13_ARTIFACTS / "02_session_summary.json") or {}
    bb13_stop = _load_json(BB13_ARTIFACTS / "12_stop_continue_quality_classification.json") or {}
    bb13_remaining = _load_json(BB13_ARTIFACTS / "16_remaining_opportunity_audit.json") or {}
    bb13_novelty = _load_json(BB13_ARTIFACTS / "20_novelty_gating_audit.json") or {}
    return {
        "available": bool(bb13_summary),
        "session_id": bb13_summary.get("session_id", "bb13-autonomous-001"),
        "experiments_used": bb13_summary.get("experiments_used"),
        "session_status": bb13_summary.get("session_status"),
        "late_mechanical_cycling": bb13_summary.get("late_mechanical_cycling"),
        "tool_distribution": bb13_summary.get("tool_distribution", {}),
        "mechanical_cycling_total": bb13_stop.get("continue_quality_counts", {}).get("MECHANICAL_CYCLING", 0),
        "unexplored_frontier_at_stop": bb13_remaining.get("frontier_unexplored_count"),
        "novelty_gating_applied_count": bb13_novelty.get("gating_applied_count", 0),
        "representation_gated_count": bb13_novelty.get("representation_gated_count", 0),
    }


def build_novelty_gating_audit_report(
    novelty_audit: List[Dict[str, Any]],
) -> Dict[str, Any]:
    class_counts = Counter(e.get("valuation_class") for e in novelty_audit)
    gated = [e for e in novelty_audit if e.get("gating_applied")]
    return {
        "research_novelty_valuation_bridge_version": RESEARCH_NOVELTY_VALUATION_BRIDGE_VERSION,
        "audit_entries": novelty_audit,
        "entry_count": len(novelty_audit),
        "gating_applied_count": len(gated),
        "valuation_class_counts": dict(class_counts),
        "representation_gated_count": class_counts.get("REPRESENTATION_NOVELTY_ONLY", 0),
        "insufficient_evidence_count": class_counts.get("INSUFFICIENT_EVIDENCE", 0),
        "total_novelty_delta": sum(float(e.get("novelty_component_delta") or 0) for e in novelty_audit),
    }


def build_bb11_bb12_scientific_comparison(
    bb12: Dict[str, Any],
    bb11: Dict[str, Any],
    ...
    mech_bb11 = bb11.get("mechanical_cycling_total") or stop_continue.get(
        "continue_quality_counts", {}
    ).get("MECHANICAL_CYCLING", 0)
    mech_bb12 = stop_continue.get("continue_quality_counts", {}).get("MECHANICAL_CYCLING", 0)
    return {
        "bb11": bb11,
        "bb12": bb12,
        "comparison_table": {
            "experiments_executed": {"bb11": bb11.get("experiments_used"), "bb12": bb12.get("experiments_used")},
            "terminal_status": {"bb11": bb11.get("session_status"), "bb12": bb12.get("session_status")},
            "late_mechanical_cycling": {
                "bb11": bb11.get("late_mechanical_cycling"),
                "bb12": bb12.get("late_mechanical_cycling"),
            },
            "mechanical_cycling_total": {"bb11": mech_bb11, "bb12": mech_bb12},
            "premature_stops": {
                "bb11": bb11.get("premature_stop_count", 0),
                "bb12": stop_continue.get("stop_quality_counts", {}).get("PREMATURE_STOP", 0),
            },
            "exit_selection_changes": {
                "bb11": bb11.get("selection_changed_by_exit_count"),
                "bb12": bb12.get("selection_changed_by_exit_count"),
            },
            "stop_competed_count": {
                "bb11": bb11.get("stop_competed_count"),
                "bb12": bb12.get("stop_competed_count"),
            },
            "unexplored_frontier_at_stop": {
                "bb11": bb11.get("unexplored_frontier_at_stop"),
                "bb12": remaining.get("frontier_unexplored_count"),
            },
            "novelty_gating_applied_count": {
                "bb11": bb11.get("novelty_gating_applied_count", 0),
                "bb12": novelty_report.get("gating_applied_count", 0),
            },
            "representation_gated_count": {
                "bb11": 0,
                "bb12": novelty_report.get("representation_gated_count", 0),
            },
            "tool_distribution": {"bb11": bb11.get("tool_distribution"), "bb12": bb12.get("tool_distribution")},
            "beneficial_interventions": {
                "bb11": (_load_json(BB11_ARTIFACTS / "13_phase_3h8_counterfactual_interventions.json") or {}).get(
                    "beneficial_count", 0
                ),
                "bb12": interventions.get("beneficial_count", 0),
            },
        },
        "not_comparable_fields": [
            "exact_decision_sequence",
            "planner_novelty_bonus",
        ],
        "primary_question": (
            "Does Phase 3H.11 reduce representation-only novelty inflation during "
            "opportunity ranking without suppressing independent research lines?"
        ),
    }
...
def evaluate_bb12_capability_gates(
    *,
    git_commit: str,
    fingerprint: str,
    ...
    novelty = trails["research_novelty_gating_audit"]
    ...
    gates["J"] = gate(
        ...
        if (
            ...
            and (
                git_commit.startswith(FROZEN_RESEARCH_COMMIT)
                or subprocess.call(
                    ["git", "merge-base", "--is-ancestor", FROZEN_RESEARCH_COMMIT, git_commit],
                    cwd=REPO,
                )
                == 0
            )
    ...
    bb11_cycling = bb11.get("mechanical_cycling_total") or bb11.get("late_mechanical_cycling") or 99
    bb12_cycling = stop_continue.get("continue_quality_counts", {}).get("MECHANICAL_CYCLING", 99)
    bb12_late = late_cycling.get("mechanical_cycling_count", 99)
    improved = bb12_cycling <= bb11_cycling
    gates["M"] = gate(
        "Mechanical cycling vs BB11",
        "PASS" if bb12_cycling < bb11_cycling else "PARTIAL" if bb12_cycling == bb11_cycling else "FAIL",
        f"bb11_cycling={bb11_cycling} bb12_cycling={bb12_cycling} bb12_late={bb12_late}",
    )
    gates["N"] = gate(
        "Novelty valuation bridge active in portfolio path",
        "PASS" if len(novelty) > 0 else "FAIL",
        f"novelty_audit_entries={len(novelty)} gating_applied={novelty_report.get('gating_applied_count', 0)}",
    )
    neg_delta = any(float(e.get("novelty_component_delta") or 0) < 0 for e in novelty)
    gates["O"] = gate(
        "No negative novelty penalty from gating",
        "PASS" if not neg_delta else "FAIL",
        f"negative_delta_entries={sum(1 for e in novelty if float(e.get('novelty_component_delta') or 0) < 0)}",
    )
    if run_error:
        for k in gates:
...
def build_session_summary(
    ...
    return {
        ...
        "novelty_gating_audit_entries": novelty_report.get("entry_count", 0),
        "novelty_gating_applied_count": novelty_report.get("gating_applied_count", 0),
        "representation_gated_count": novelty_report.get("representation_gated_count", 0),
        "capability_gates": {k: v["result"] for k, v in gates.items()},
    }


def build_bb12_report(
    ...
    if run_error:
        ...
        lines.extend(
            [
                ...
                f"- Novelty gating audit entries: {summary.get('novelty_gating_audit_entries')}.",
                f"- Novelty gating applied: {summary.get('novelty_gating_applied_count')} times.",
                f"- Representation-only gated: {summary.get('representation_gated_count')}.",
    ...
    lines.extend(
        [
            ...
            "",
            f"- Valuation class counts: `{novelty_report.get('valuation_class_counts')}`",
            f"- Total novelty delta: {novelty_report.get('total_novelty_delta')}",
            "",
            "### BB11 vs BB12",
            "",
        ]
    ...
    for metric, vals in table.items():
        if isinstance(vals, dict) and "bb11" in vals and "bb12" in vals:
            lines.append(f"- {metric}: BB11={vals['bb11']} | BB12={vals['bb12']}")
    lines.extend(["", "### Capability Gates (A–O)", ""])
    ...
    premature = stop_continue.get("stop_quality_counts", {}).get("PREMATURE_STOP", 0)
    mech_bb11 = comparison.get("comparison_table", {}).get("mechanical_cycling_total", {}).get("bb11")
    mech_bb12 = comparison.get("comparison_table", {}).get("mechanical_cycling_total", {}).get("bb12")
    gated = novelty_report.get("gating_applied_count", 0)
    if run_error:
        lines.append("- Run failure prevents scientific inference.")
    elif gated > 0 and (mech_bb12 or 99) <= (mech_bb11 or 99) and premature <= 1:
        lines.append(
            "- Novelty gating is active in live ranking; mechanical cycling did not increase versus BB11."
        )
    elif gated == 0:
        lines.append(
        ...
        lines.append("- Mixed signals; see comparison table and novelty audit for detail.")
    if premature > 0:
...
def main() -> None:
    ...
    print("=== BLIND BENCHMARK 14: verify frozen commit ===")
    git_commit = _verify_frozen_commit()
    print(f"Commit verified: {git_commit}")

    print("=== Load frozen panel (BB01–BB11 fingerprint) ===")
    ...
    manifest = build_bb12_freeze_manifest(panel, inventory, preflight, git_commit)
    ...
    print(f"=== Autonomous research session (budget=12, Phase 3H.11 @ {FROZEN_RESEARCH_COMMIT}) ===")
    ...
    if result is None:
        ...
        (ARTIFACTS / "BB12_REPORT.md").write_text(
            build_bb12_report(failure, {}, {}, {}, {}, {}, run_error),
            encoding="utf-8",
        )
    ...
    novelty_report = build_novelty_gating_audit_report(trails["research_novelty_gating_audit"])
    rank_recon_report = build_rank_reconciliation_audit_report(trails["research_rank_reconciliation_audit"])
    bb11 = load_bb11_baseline()
    bb13 = load_bb13_baseline()
    ...
    bb14_baseline = {
        ...
        "tool_distribution": dict(tool_dist),
        "novelty_gating_applied_count": novelty_report.get("gating_applied_count", 0),
        "rank_reconciliation_applied_count": rank_recon_report.get("reconciliation_applied_count", 0),
        "representation_only_rank_reconciliations": rank_recon_report.get("representation_only_reconciliations", 0),
    }
    comparison = build_bb11_bb12_scientific_comparison(
        bb14_baseline, bb11, stop_continue, interventions, novelty_report, remaining
    )
    comparison["bb13"] = bb13
    comparison["bb14"] = bb14_baseline
    comparison["four_way"] = {
        "mechanical_cycling": {
            "bb11": bb11.get("mechanical_cycling_total"),
            "bb13": bb13.get("mechanical_cycling_total"),
            "bb14": stop_continue.get("continue_quality_counts", {}).get("MECHANICAL_CYCLING", 0),
        },
        "novelty_gating_applied": {
            "bb11": 0,
            "bb13": bb13.get("novelty_gating_applied_count", 0),
            "bb14": novelty_report.get("gating_applied_count", 0),
        },
        "rank_reconciliation_applied": {
            "bb11": 0,
            "bb13": 0,
            "bb14": rank_recon_report.get("reconciliation_applied_count", 0),
        },
        "unexplored_frontier_at_stop": {
            "bb11": bb11.get("unexplored_frontier_at_stop"),
            "bb13": bb13.get("unexplored_frontier_at_stop"),
            "bb14": remaining.get("frontier_unexplored_count"),
        },
        "tool_distribution": {
            "bb11": bb11.get("tool_distribution"),
            "bb13": bb13.get("tool_distribution"),
            "bb14": dict(tool_dist),
        },
    }
    gates = evaluate_bb12_capability_gates(
        git_commit=git_commit,
        fingerprint=REQUIRED_FINGERPRINT,
        run_error=run_error,
        result=result,
    ...
    summary = build_session_summary(
        result, git_commit, trails, stop_continue, interventions, late_cycling, gates, novelty_report
    )
    ...
    writes = {
        ...
        "14_bb11_bb12_scientific_comparison.json": comparison,
        ...
        "20_novelty_gating_audit.json": novelty_report,
        "21_rank_reconciliation_audit.json": rank_recon_report,
    }
    for fname, payload in writes.items():
        _write_json(ARTIFACTS / fname, payload)

    report = build_bb12_report(
        summary, comparison, stop_continue, interventions, gates, novelty_report, run_error
    )
    (ARTIFACTS / "BB14_REPORT.md").write_text(report, encoding="utf-8")
