#!/usr/bin/env python3
"""
Blind Benchmark 08 — post-3G.4.2 budget termination lifecycle validation & deduplication measurement.

Orchestration and reporting ONLY. Does NOT modify research logic.
Uses the SAME frozen panel fingerprint as BB01–BB07.
Primary test: does budget-termination repair eliminate the BB08 post-budget ERROR lifecycle defect
ResearchGraphError and terminate with BUDGET_EXHAUSTED after 12/12 experiments
...
BB07_ARTIFACTS = REPO / "benchmarks" / "blind_benchmark_07" / "artifacts"
BB05_GIT_REF = "cursor/blind-benchmark-05-aad2"
BB06_GIT_REF = "cursor/blind-benchmark-06-aad2"
BB07_GIT_REF = "cursor/blind-benchmark-07-aad2"
...
BENCHMARK_ID = "blind_benchmark_08"
BENCHMARK_VERSION = "bb08_v1"
SESSION_ID = "bb08-autonomous-001"
FROZEN_RESEARCH_COMMIT = "c605c8326"
...
def build_freeze_manifest(panel: pd.DataFrame, inventory: Dict[str, Any], preflight: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "budget_termination_commit": FROZEN_RESEARCH_COMMIT,
...
def _load_bb07_artifact(name: str) -> Any:
    """Load frozen BB07 artifact from workspace or git ref — read-only."""
    path = BB07_ARTIFACTS / name
    if path.exists():
        return json.loads(path.read_text(encoding="utf-8"))
    try:
        raw = subprocess.check_output(
            ["git", "show", f"{BB07_GIT_REF}:benchmarks/blind_benchmark_07/artifacts/{name}"],
            cwd=REPO,
            text=True,
        )
        return json.loads(raw)
...
def _spec_hash_from_diary_entry(entry: Dict[str, Any]) -> Dict[str, str]:
    """Stable semantic hashes for population/outcome specs in diary entries."""
    from modules.edge_research.research_grammar import OutcomeSpec, PopulationSpec

    pop = entry.get("population_spec") or {}
    out = entry.get("outcome_spec") or {}
    pop_h = PopulationSpec.from_dict(pop).content_hash() if pop else ""
    out_h = OutcomeSpec.from_dict(out).content_hash() if out else ""
    return {"population_spec_hash": pop_h, "outcome_spec_hash": out_h}


def _diary_semantics_signature(entry: Dict[str, Any]) -> Dict[str, Any]:
    hashes = _spec_hash_from_diary_entry(entry)
    inputs = dict(entry.get("tool_inputs") or {})
    return {
        "step": entry.get("step"),
        "tool": entry.get("tool_selected"),
        "tool_inputs": inputs,
        "feature": inputs.get("feature_column") or inputs.get("partition_column") or "",
        "population_spec_hash": hashes["population_spec_hash"],
        "outcome_spec_hash": hashes["outcome_spec_hash"],
        "observation_horizon": entry.get("observation_horizon"),
        "frame_id": entry.get("frame_id"),
        "branch_root_id": entry.get("current_branch_root"),
    }


def _global_decision_signature(entry: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "decision_index": entry.get("decision_index"),
        "triggering_experiment_node_id": entry.get("triggering_experiment_node_id"),
        "resulting_experiment_node_id": entry.get("resulting_experiment_node_id"),
        "selected_source": entry.get("selected_source"),
        "selected_action_id": entry.get("selected_action_id"),
        "selected_erv": entry.get("selected_erv"),
        "best_local_erv": entry.get("best_local_erv"),
        "best_frontier_erv": entry.get("best_frontier_erv"),
        "best_deferred_erv": entry.get("best_deferred_erv"),
        "best_global_alternative_erv": entry.get("best_global_alternative_erv"),
        "global_opportunity_cost": entry.get("global_opportunity_cost"),
        "branch_before": entry.get("branch_before"),
        "branch_after": entry.get("branch_after"),
        "frame_before": entry.get("frame_before"),
        "observation_horizon_before": entry.get("observation_horizon_before"),
        "budget_remaining": entry.get("budget_remaining"),
    }


def build_bb07_bb08_lifecycle_comparison(
    bb08_summary: Dict[str, Any],
    bb08_graph: Any,
) -> Dict[str, Any]:
    """Causal comparison: did 3G.4.2 fix BB07 post-budget ERROR lifecycle defect?"""
    bb07_summary = _load_bb07_artifact("09_run_summary.json") or {}
    bb07_stop = bb07_summary.get("session_stop_reason") or {}

    bb08_stop = bb08_summary.get("session_stop_reason") or {}
    if not bb08_stop and bb08_graph is not None:
        bb08_stop = bb08_graph.session.session_stop_reason or {}

    bb08_used = bb08_summary.get("experiments_used", 0)
    bb08_status = bb08_summary.get("session_status", "")

    lifecycle_repair = (
        bb08_used >= EXPERIMENT_BUDGET
        and bb08_status == "BUDGET_EXHAUSTED"
        and bb07_summary.get("session_status") == "ERROR"
        and bb07_stop.get("error_type") == "ResearchGraphError"
        and "budget exhausted" in str(bb07_stop.get("error_message", "")).lower()
    )

    return {
        "comparison_type": "bb07_bb08_lifecycle_repair_causal",
        "bb07_frozen_commit": "dd027b3da",
        "bb08_frozen_commit": FROZEN_RESEARCH_COMMIT,
        "primary_question": (
            "Did Phase 3G.4.2 repair the BB07 post-budget lifecycle defect "
            "(ERROR/ResearchGraphError after experiment 12)?"
        ),
        "bb07_terminal_state": {
            "session_status": bb07_summary.get("session_status"),
            "experiments_used": bb07_summary.get("experiments_used"),
            "stop_code": bb07_stop.get("code"),
            "error_type": bb07_stop.get("error_type"),
            "error_message": bb07_stop.get("error_message"),
            "operation": bb07_stop.get("operation"),
        },
        "bb08_terminal_state": {
            "session_status": bb08_status,
            "experiments_used": bb08_used,
            "stop_code": bb08_stop.get("code"),
            "terminal_status": bb08_stop.get("terminal_status"),
            "experiment_budget": bb08_stop.get("experiment_budget"),
            "experiments_executed": bb08_stop.get("experiments_executed"),
            "remaining_budget": bb08_stop.get("remaining_budget"),
            "final_experiment_id": bb08_stop.get("final_experiment_id"),
            "unexplored_frontier_count": bb08_stop.get("unexplored_frontier_count"),
            "unresolved_promising_deferred_count": bb08_stop.get(
                "unresolved_promising_deferred_count"
            ),
        },
        "lifecycle_repair_verdict": {
            "bb07_post_budget_error_eliminated": lifecycle_repair,
            "bb08_completed_12_experiments": bb08_used >= EXPERIMENT_BUDGET,
            "bb08_budget_exhausted_status": bb08_status == "BUDGET_EXHAUSTED",
            "bb08_no_research_graph_error": bb08_stop.get("error_type") is None,
            "causal_narrow_conclusion": "REPAIR_CONFIRMED" if lifecycle_repair else "REPAIR_NOT_CONFIRMED",
        },
        "interpretation_guidance": (
            "BB08 success is determined by clean BUDGET_EXHAUSTED termination, "
            "not by edge discovery or benchmark score."
        ),
    }


def build_bb07_bb08_decision_parity(
    bb08_diary: List[Dict[str, Any]],
    bb08_global_diary: List[Dict[str, Any]],
    bb08_graph: Any,
) -> Dict[str, Any]:
    """Compare experiments/decisions 1–12 research semantics between BB07 and BB08."""
    bb07_diary = _load_bb07_artifact("03_research_diary.json") or []
    bb07_global = _load_bb07_artifact("28_global_allocation_diary.json") or []

    from modules.edge_research.research_state import compute_experiment_content_hash, ExperimentSpec

    experiment_rows: List[Dict[str, Any]] = []
    experiment_mismatches: List[Dict[str, Any]] = []

    for step in range(1, EXPERIMENT_BUDGET + 1):
        b7 = next((d for d in bb07_diary if d.get("step") == step), None)
        b8 = next((d for d in bb08_diary if d.get("step") == step), None)
        sig7 = _diary_semantics_signature(b7) if b7 else None
        sig8 = _diary_semantics_signature(b8) if b8 else None

        content_hash_bb08 = ""
        if b8 and bb08_graph is not None:
            node = bb08_graph.nodes.get(b8.get("experiment_node_id", ""))
            if node and node.experiment_spec:
                content_hash_bb08 = compute_experiment_content_hash(node.experiment_spec)

        row = {
            "step": step,
            "bb07_experiment_node_id": (b7 or {}).get("experiment_node_id"),
            "bb08_experiment_node_id": (b8 or {}).get("experiment_node_id"),
            "bb07_semantics": sig7,
            "bb08_semantics": sig8,
            "bb08_content_hash": content_hash_bb08,
            "semantics_match": sig7 == sig8 if sig7 and sig8 else False,
        }
        experiment_rows.append(row)
        if sig7 and sig8 and sig7 != sig8:
            experiment_mismatches.append(row)

    decision_rows: List[Dict[str, Any]] = []
    decision_mismatches: List[Dict[str, Any]] = []
    max_decisions = max(len(bb07_global), len(bb08_global_diary))
    for idx in range(1, max_decisions + 1):
        d7 = next((d for d in bb07_global if d.get("decision_index") == idx), None)
        d8 = next((d for d in bb08_global_diary if d.get("decision_index") == idx), None)
        sig7 = _global_decision_signature(d7) if d7 else None
        sig8 = _global_decision_signature(d8) if d8 else None
        match = sig7 == sig8 if sig7 and sig8 else (d7 is None and d8 is None)
        row = {
            "decision_index": idx,
            "bb07": sig7,
            "bb08": sig8,
        ...
        if not match and (d7 or d8):
            decision_mismatches.append(row)

    all_experiments_match = len(experiment_mismatches) == 0
    all_decisions_match = len(decision_mismatches) == 0

    return {
        "comparison_type": "bb07_bb08_research_semantics_parity",
        "bb07_frozen_commit": "dd027b3da",
        "bb08_frozen_commit": FROZEN_RESEARCH_COMMIT,
        "experiment_parity": {
            "compared_steps": EXPERIMENT_BUDGET,
            "all_match": all_experiments_match,
            "mismatch_count": len(experiment_mismatches),
            "mismatches": experiment_mismatches,
            "rows": experiment_rows,
        },
        "global_decision_parity": {
            "bb07_decision_count": len(bb07_global),
            "bb08_decision_count": len(bb08_global_diary),
            "all_match": all_decisions_match,
            "mismatch_count": len(decision_mismatches),
            "mismatches": decision_mismatches,
            "rows": decision_rows,
        },
        "parity_verdict": {
            "research_semantics_unchanged": all_experiments_match and all_decisions_match,
            "note": (
                "Decision count may differ by one if BB08 skips post-final-experiment planning; "
                "experiment steps 1–12 must match if lifecycle refactor did not alter research behavior."
            ),
        },
    }


def build_final_experiment_audit(
    result: Any,
    diary: List[Dict[str, Any]],
    global_diary: List[Dict[str, Any]],
) -> Dict[str, Any]:
    """Prove experiment 12 appears exactly once across audit surfaces."""
    graph = result.graph
    experiments = [
    ...
    if len(experiments) < EXPERIMENT_BUDGET:
        final_exp = experiments[-1] if experiments else None
    else:
        final_exp = experiments[EXPERIMENT_BUDGET - 1]

    final_id = final_exp.node_id if final_exp else ""
    from modules.edge_research.research_state import compute_experiment_content_hash

    content_hash = (
        compute_experiment_content_hash(final_exp.experiment_spec)
        if final_exp and final_exp.experiment_spec
        else ""
    )

    ledger = graph.get_search_accounting().session_ledger
    hash_occurrences = sum(1 for h, nid in graph.experiment_index.items() if nid == final_id)

    diary_entries = [d for d in diary if d.get("experiment_node_id") == final_id]
    global_refs = [
        d for d in global_diary
        if d.get("triggering_experiment_node_id") == final_id
        or d.get("resulting_experiment_node_id") == final_id
    ]
    ...
    selected_after_final = [
        ...
        and item.enqueued_sequence > (final_exp.created_at if final_exp else "")
    ]

    stop = graph.session.session_stop_reason or {}
    deferred_count = stop.get("unresolved_promising_deferred_count", 0)
    unexplored = frontier.unexplored_items()
    deferred_branches = sum(
        1
        for b in graph.get_portfolio_state().branches.values()
        if b.status == "DEFERRED_PROMISING"
    )

    return {
        "final_experiment_step": EXPERIMENT_BUDGET,
        "final_experiment_node_id": final_id,
        "final_experiment_content_hash": content_hash,
        "appears_in_research_graph": final_id in graph.nodes,
        "experiment_result_finalized": bool(
            final_exp and final_exp.experiment_result and final_exp.experiment_result.finalized
        ),
        "experiment_index_occurrences": hash_occurrences,
        "identity_ledger_unique": hash_occurrences == 1,
        "search_accounting_experiments_executed": ledger.experiments_executed,
        "diary_entry_count": len(diary_entries),
        "global_diary_reference_count": len(global_refs),
        "portfolio_metrics_present": bool(graph.get_portfolio_state().metrics),
        "frame_registry_present": bool(graph.session.research_frames),
        "no_post_final_selected_opportunities": len(selected_after_final) == 0,
        "post_final_selected_frontier_ids": selected_after_final,
        "terminal_stop_reason": stop,
        "remaining_opportunities_at_exhaustion": {
            "unexplored_frontier_count": len(unexplored),
            "unexplored_frontier_sample": [
                {
                    "frontier_id": i.frontier_id,
                    "action_code": i.action_code,
                    "status": i.status,
                    "planner_score": i.planner_score,
                }
                for i in sorted(unexplored, key=lambda x: -x.planner_score)[:10]
            ],
            "deferred_promising_branch_count": deferred_branches,
            "stop_reason_deferred_count": deferred_count,
        },
    }


def evaluate_bb08_lifecycle_gates(
    lifecycle_comparison: Dict[str, Any],
    final_audit: Dict[str, Any],
    parity: Dict[str, Any],
) -> Dict[str, Any]:
    """BB08 lifecycle validation gates — primary success criteria."""
    verdict = lifecycle_comparison.get("lifecycle_repair_verdict") or {}
    def _gate(ok: bool) -> str:
        return "PASS" if ok else "FAIL"

    exp12_ok = bool(
        final_audit.get("identity_ledger_unique")
        and final_audit.get("experiment_result_finalized")
    )
    return {
        "gate_A_twelve_experiments_executed": {
            "result": _gate(bool(verdict.get("bb08_completed_12_experiments"))),
            "evidence": final_audit.get("search_accounting_experiments_executed"),
        },
        "gate_B_budget_exhausted_terminal_status": {
            "result": _gate(bool(verdict.get("bb08_budget_exhausted_status"))),
            "evidence": lifecycle_comparison.get("bb08_terminal_state"),
        },
        "gate_C_no_research_graph_error": {
            "result": _gate(bool(verdict.get("bb08_no_research_graph_error"))),
            "evidence": lifecycle_comparison.get("bb08_terminal_state", {}).get("stop_code"),
        },
        "gate_D_final_experiment_persisted_once": {
            "result": _gate(exp12_ok),
            "evidence": {
                "final_id": final_audit.get("final_experiment_node_id"),
                "index_occurrences": final_audit.get("experiment_index_occurrences"),
            },
        },
        "gate_E_no_post_final_selected_opportunities": {
            "result": _gate(bool(final_audit.get("no_post_final_selected_opportunities"))),
            "evidence": final_audit.get("post_final_selected_frontier_ids"),
        },
        "gate_F_research_semantics_parity_bb07": {
            "result": _gate(
                bool(parity.get("parity_verdict", {}).get("research_semantics_unchanged"))
            ),
            "evidence": {
                "experiment_mismatches": parity.get("experiment_parity", {}).get("mismatch_count"),
                "decision_mismatches": parity.get("global_decision_parity", {}).get("mismatch_count"),
            },
        },
        "gate_G_lifecycle_repair_confirmed": {
            "result": _gate(verdict.get("causal_narrow_conclusion") == "REPAIR_CONFIRMED"),
            "evidence": verdict,
        },
    }


def build_bb08_post_run_freeze_manifest(
    summary: Dict[str, Any],
    lifecycle_comparison: Dict[str, Any],
    lifecycle_gates: Dict[str, Any],
    parity: Dict[str, Any],
    final_audit: Dict[str, Any],
) -> Dict[str, Any]:
    """Immutable post-run artifact freeze for BB08 lifecycle validation."""
    stop = summary.get("session_stop_reason") or {}
    verdict = lifecycle_comparison.get("lifecycle_repair_verdict") or {}
    artifact_files = sorted(p.name for p in ARTIFACTS.iterdir() if p.is_file())
    return {
        "benchmark_id": BENCHMARK_ID,
        "benchmark_version": BENCHMARK_VERSION,
        "freeze_type": "post_run_artifact_freeze",
        "frozen_at": _utc_now(),
        "frozen_research_commit": FROZEN_RESEARCH_COMMIT,
        "budget_termination_commit": FROZEN_RESEARCH_COMMIT,
        "experiment_identity_version": EXPERIMENT_IDENTITY_VERSION,
        "global_allocator_version": GLOBAL_ALLOCATOR_VERSION,
        "dataset_fingerprint_sha256": REQUIRED_FINGERPRINT,
        "experiments_completed": summary.get("experiments_used"),
        "experiments_budget": EXPERIMENT_BUDGET,
        "session_id": SESSION_ID,
        "session_status_at_freeze": summary.get("session_status"),
        "session_stop_reason_code": stop.get("code"),
        "session_stop_detail": stop.get("detail") or stop.get("error_message"),
        "terminal_status": stop.get("terminal_status"),
        "final_experiment_id": stop.get("final_experiment_id") or final_audit.get(
            "final_experiment_node_id"
        ),
        "unexplored_frontier_count": stop.get("unexplored_frontier_count"),
        "unresolved_promising_deferred_count": stop.get("unresolved_promising_deferred_count"),
        "research_logic_modified_after_run": False,
        "prior_benchmarks_immutable": [
            "blind_benchmark_01",
            "blind_benchmark_02",
            "blind_benchmark_03",
            "blind_benchmark_04",
        ...
        "lifecycle_repair_primary_verdict": verdict.get("causal_narrow_conclusion"),
        "lifecycle_repair_notes": [
            "BB07: 12/12 experiments executed but session ended ERROR with ResearchGraphError on post-final plan_after_experiment.",
            "BB08 primary question: did Phase 3G.4.2 terminate cleanly with BUDGET_EXHAUSTED after experiment 12?",
            f"BB08 terminal status: {summary.get('session_status')}",
            f"BB08 stop code: {stop.get('code')}",
            f"Research semantics parity BB07→BB08: {parity.get('parity_verdict', {}).get('research_semantics_unchanged')}",
        ],
        "bb08_lifecycle_gates": {k: v["result"] for k, v in lifecycle_gates.items()},
        "artifact_count": len(artifact_files),
        "artifact_files": artifact_files,
        "modification_policy": "IMMUTABLE_HISTORICAL_EVIDENCE_NO_RERUN",
    }
...
def build_global_allocation_metrics(
    ...
    """Aggregate BB08 allocation-quality measurements."""
    graph = result.graph
    frontier = graph.get_frontier()
    portfolio = graph.get_portfolio_state()
...
def build_bb06_bb07_identity_repair_comparison(
    ...
    bb07_decision_4 = next(
        (d for d in bb08_global_diary if d.get("decision_index") == 4),
        {},
    )
...
def build_bb05_bb08_allocation_comparison(
    bb08_coverage: Dict[str, Any],
    bb08_metrics: Dict[str, Any],
    bb08_global_diary: List[Dict[str, Any]],
    bb08_portfolio_metrics: Any,
) -> Dict[str, Any]:
    """Compare BB08 vs BB05 on allocation quality — secondary to identity repair verdict."""
    ...
    bb08_trad = bb08_coverage.get("traditional_coverage", bb08_coverage)
    bb05_fc = bb05_cov.get("frame_coverage", bb05_cov)
    bb08_fc = bb08_coverage.get("frame_coverage", bb08_coverage)

    bb05_decisions = [d for d in bb05_diary if d.get("portfolio_decision")]
    bb08_global_decisions = bb08_global_diary

    def row(metric: str, bb5: Any, bb7: Any, note: str = "") -> Dict[str, Any]:
        return {"metric": metric, "bb05": bb5, "bb08": bb7, "allocation_note": note}
    ...
    return {
        "comparison_type": "research_allocation_quality_bb05_vs_bb08",
        "bb05_frozen_commit": "61b669927",
        "bb08_frozen_commit": FROZEN_RESEARCH_COMMIT,
        "bb08_budget_termination_commit": FROZEN_RESEARCH_COMMIT,
        "primary_question": (
            "If BB08 completes normally, does full-session evidence support or weaken "
            "the Phase 3G.4 Global Allocator hypothesis relative to BB05?"
        ),
        "rows": [
            row("experiments_executed", bb05_trad.get("experiments_executed"), bb08_trad.get("experiments_executed")),
            row(
                "features_touched",
                len(bb05_trad.get("features_touched", [])),
                len(bb08_trad.get("features_touched", [])),
                "Breadth alone is not success criterion",
            ),
            row(
                "frames_executed",
                bb05_fc.get("frames_executed"),
                bb08_fc.get("frames_executed"),
            ),
            row(
                "global_context_switches",
                0,
                bb08_metrics.get("global_context_switch_count"),
                "BB05 had no global allocator; switches only via STOP_BRANCH→frontier",
            ),
            row(
                "portfolio_decisions_audited",
                len(bb05_decisions),
                len(bb08_global_decisions),
            ),
            row(
                "negative_local_selections",
                bb05_neg_local_miss,
                bb08_metrics.get("negative_local_with_positive_global_alternative"),
                "BB08 target: fewer negative-local picks when positive global alternative exists",
            ),
            row(
                "high_value_unexplored_at_end_historical",
                bb05_frontier.get("high_value_unexplored"),
                bb08_metrics.get("unresolved_high_value_count"),
                "Uses historical planner_score threshold >2.0",
            ),
            row(
                "revisit_count",
                bb05_metrics.get("revisit_count", 0),
                bb08_metrics.get("genuine_revisit_count"),
            ),
            row(
                ...
                (
                    sum(d.get("global_opportunity_cost", 0) for d in bb08_global_decisions)
                    / max(1, len(bb08_global_decisions))
                ),
            ),
            row(
                "strong_local_protected",
                "N/A",
                bb08_metrics.get("strong_local_beats_frontier_count"),
                "Local continuation when local current ERV genuinely highest",
            ),
            row(
                "frontier_interrupts_viable_local",
                "N/A",
                bb08_metrics.get("frontier_interrupts_viable_local"),
                "Frontier selected while viable local branch existed",
            ),
        ],
        "bb05_session_status": bb05_summary.get("session_status"),
        "bb08_selection_sources": bb08_metrics.get("selection_source_counts"),
        "interpretation_guidance": (
            "Only evaluate full-session allocation improvement if BB08 completed 12 experiments. "
...
def evaluate_global_allocator_capability_gates(
    ...
    """BB08 capability gates: identity repair + global allocation behavior."""
...
def main() -> None:
    ...
    print("=== BLIND BENCHMARK 08: load frozen panel (BB01–BB06 fingerprint) ===")
    ...
    print("=== Autonomous research session (budget=%d, Phase 3G.4.2) ===" % EXPERIMENT_BUDGET)
    run_error: Optional[str] = None
    result = None
    try:
    ...
    if result is None:
        failure_report = {
            ...
            "interpretation": (
                ...
                "Research logic was NOT modified post-failure per BB08 policy."
            ),
        }
    ...
    bb05_bb08_comparison = build_bb05_bb08_allocation_comparison(
    ...
    lifecycle_comparison = build_bb07_bb08_lifecycle_comparison(bb08_summary_pre, result.graph)
    decision_parity = build_bb07_bb08_decision_parity(diary, global_diary, result.graph)
    final_audit = build_final_experiment_audit(result, diary, global_diary)
    lifecycle_gates = evaluate_bb08_lifecycle_gates(lifecycle_comparison, final_audit, decision_parity)
    ...
    (ARTIFACTS / "16_capability_gates.json").write_text(json.dumps(gates, indent=2, default=str), encoding="utf-8")
    (ARTIFACTS / "17_bb01_bb08_comparison.json").write_text(
        json.dumps(comparison, indent=2, default=str), encoding="utf-8"
    )
    (ARTIFACTS / "19_horizon_survivability_audit.json").write_text(
        json.dumps(survivability_audit, indent=2, default=str), encoding="utf-8"
    ...
    (ARTIFACTS / "30_bb05_bb08_allocation_comparison.json").write_text(
        json.dumps(bb05_bb08_comparison, indent=2, default=str), encoding="utf-8"
    )
    ...
    (ARTIFACTS / "35_bb07_bb08_lifecycle_comparison.json").write_text(
        json.dumps(lifecycle_comparison, indent=2, default=str), encoding="utf-8"
    )
    (ARTIFACTS / "36_bb07_bb08_decision_parity.json").write_text(
        json.dumps(decision_parity, indent=2, default=str), encoding="utf-8"
    )
    (ARTIFACTS / "37_final_experiment_audit.json").write_text(
        json.dumps(final_audit, indent=2, default=str), encoding="utf-8"
    )
    (ARTIFACTS / "38_bb08_lifecycle_capability_gates.json").write_text(
        json.dumps(lifecycle_gates, indent=2, default=str), encoding="utf-8"
    )
    ...
    summary["lifecycle_repair_verdict"] = lifecycle_comparison.get("lifecycle_repair_verdict")
    summary["bb08_lifecycle_gates"] = {k: v["result"] for k, v in lifecycle_gates.items()}
    summary["bb07_bb08_decision_parity"] = decision_parity.get("parity_verdict")
    summary["final_experiment_audit"] = {
        "final_experiment_node_id": final_audit.get("final_experiment_node_id"),
        "identity_ledger_unique": final_audit.get("identity_ledger_unique"),
        "no_post_final_selected_opportunities": final_audit.get("no_post_final_selected_opportunities"),
        "remaining_opportunities": final_audit.get("remaining_opportunities_at_exhaustion"),
    }
    ...
    post_run_freeze = build_bb08_post_run_freeze_manifest(
        summary, lifecycle_comparison, lifecycle_gates, decision_parity, final_audit
    )
    (ARTIFACTS / "39_bb08_post_run_freeze_manifest.json").write_text(
        json.dumps(post_run_freeze, indent=2, default=str), encoding="utf-8"
    )
    print("=== BLIND BENCHMARK 08 COMPLETE ===")
