#!/usr/bin/env python3
"""
Blind Benchmark 06 — post-3G.4 global research allocator measurement.

Orchestration and reporting ONLY. Does NOT modify research logic.
Uses the SAME frozen panel fingerprint as BB01–BB05.
Primary test: does global research allocation materially improve allocation
of scarce research budget relative to BB05?
"""
...
BENCHMARK_ID = "blind_benchmark_06"
BENCHMARK_VERSION = "bb06_v1"
SESSION_ID = "bb06-autonomous-001"
FROZEN_RESEARCH_COMMIT = "94bde8807"
...
def build_global_allocation_metrics(
    ...
    """Aggregate BB06 allocation-quality measurements."""
    graph = result.graph
    frontier = graph.get_frontier()
    portfolio = graph.get_portfolio_state()
...
def build_bb05_bb06_allocation_comparison(
    bb06_coverage: Dict[str, Any],
    bb06_metrics: Dict[str, Any],
    bb06_global_diary: List[Dict[str, Any]],
    bb06_portfolio_metrics: Any,
) -> Dict[str, Any]:
    """Compare BB06 vs BB05 on allocation quality — not switch count or breadth alone."""
    ...
    bb06_trad = bb06_coverage.get("traditional_coverage", bb06_coverage)
    bb05_fc = bb05_cov.get("frame_coverage", bb05_cov)
    bb06_fc = bb06_coverage.get("frame_coverage", bb06_coverage)

    bb05_decisions = [d for d in bb05_diary if d.get("portfolio_decision")]
    bb06_global_decisions = bb06_global_diary

    def row(metric: str, bb5: Any, bb6: Any, note: str = "") -> Dict[str, Any]:
        return {"metric": metric, "bb05": bb5, "bb06": bb6, "allocation_note": note}
    ...
    return {
        "comparison_type": "research_allocation_quality_bb05_vs_bb06",
        "bb05_frozen_commit": "61b669927",
        "bb06_frozen_commit": FROZEN_RESEARCH_COMMIT,
        "primary_question": (
            "Does global research allocation materially improve allocation of scarce "
            "research budget relative to BB05?"
        ),
        "rows": [
            row("experiments_executed", bb05_trad.get("experiments_executed"), bb06_trad.get("experiments_executed")),
            row(
                "features_touched",
                len(bb05_trad.get("features_touched", [])),
                len(bb06_trad.get("features_touched", [])),
                "Breadth alone is not success criterion",
            ),
            row(
                "frames_executed",
                bb05_fc.get("frames_executed"),
                bb06_fc.get("frames_executed"),
            ),
            row(
                "global_context_switches",
                0,
                bb06_metrics.get("global_context_switch_count"),
                "BB05 had no global allocator; switches only via STOP_BRANCH→frontier",
            ),
            row(
                "portfolio_decisions_audited",
                len(bb05_decisions),
                len(bb06_global_decisions),
            ),
            row(
                "negative_local_selections",
                bb05_neg_local_miss,
                bb06_metrics.get("negative_local_with_positive_global_alternative"),
                "BB06 target: fewer negative-local picks when positive global alternative exists",
            ),
            row(
                "high_value_unexplored_at_end_historical",
                bb05_frontier.get("high_value_unexplored"),
                bb06_metrics.get("unresolved_high_value_count"),
                "Uses historical planner_score threshold >2.0",
            ),
            row(
                "revisit_count",
                bb05_metrics.get("revisit_count", 0),
                bb06_metrics.get("genuine_revisit_count"),
            ),
            row(
                ...
                (
                    sum(d.get("global_opportunity_cost", 0) for d in bb06_global_decisions)
                    / max(1, len(bb06_global_decisions))
                ),
            ),
            row(
                "strong_local_protected",
                "N/A",
                bb06_metrics.get("strong_local_beats_frontier_count"),
                "Local continuation when local current ERV genuinely highest",
            ),
            row(
                "frontier_interrupts_viable_local",
                "N/A",
                bb06_metrics.get("frontier_interrupts_viable_local"),
                "Frontier selected while viable local branch existed",
            ),
        ],
        "bb05_session_status": bb05_summary.get("session_status"),
        "bb06_selection_sources": bb06_metrics.get("selection_source_counts"),
        "interpretation_guidance": (
            "Switching more is not inherently better. Staying local is correct when "
            "local current ERV is genuinely highest. Compare on allocation quality metrics."
        ),
    }
...
def evaluate_global_allocator_capability_gates(
    ...
    """BB06 capability gates focused on global allocation behavior."""
...
def main() -> None:
    ...
    print("=== BLIND BENCHMARK 06: load frozen panel (BB01–BB05 fingerprint) ===")
    ...
    print("=== Autonomous research session (budget=%d, Phase 3G.4) ===" % EXPERIMENT_BUDGET)
    run_error: Optional[str] = None
    result = None
    try:
    ...
    if result is None:
        failure_report = {
            ...
            "interpretation": (
                ...
                "Research logic was NOT modified post-failure per BB06 policy."
            ),
        }
    ...
    (ARTIFACTS / "16_capability_gates.json").write_text(json.dumps(gates, indent=2, default=str), encoding="utf-8")
    (ARTIFACTS / "17_bb01_bb06_comparison.json").write_text(
        json.dumps(comparison, indent=2, default=str), encoding="utf-8"
    )
    (ARTIFACTS / "19_horizon_survivability_audit.json").write_text(
        json.dumps(survivability_audit, indent=2, default=str), encoding="utf-8"
    ...
    (ARTIFACTS / "30_bb05_bb06_allocation_comparison.json").write_text(
        json.dumps(bb05_bb06_comparison, indent=2, default=str), encoding="utf-8"
    )
    ...
    print("=== BLIND BENCHMARK 06 COMPLETE ===")
