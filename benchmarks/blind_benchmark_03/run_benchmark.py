#!/usr/bin/env python3
"""
Blind Benchmark 03 — post-3G.2 autonomous reframing measurement.

Orchestration and reporting ONLY. Does NOT modify research logic.
Uses the SAME frozen panel fingerprint as BB01/BB02.
"""
...
BENCHMARK_ID = "blind_benchmark_03"
BENCHMARK_VERSION = "bb03_v1"
SESSION_ID = "bb03-autonomous-001"
...
def build_freeze_manifest(panel: pd.DataFrame, inventory: Dict[str, Any], preflight: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "prior_benchmarks": ["blind_benchmark_01", "blind_benchmark_02"],
        ...
        "dataset_fingerprint_sha256": REQUIRED_FINGERPRINT,
        "fingerprint_verified_against_bb01_bb02": True,
        ...
        "phases_frozen": ["3D", "3E", "3F", "3G", "3G.1", "3G.2"],
...
def evaluate_capability_gates(
    ...
    gate_c = any(t.get("execution", {}).get("was_researched") for t in transitions if t.get("trigger"))
    gate_d = temporal_audit.get("all_executed_frames_temporally_legal", True)
    gate_e = trad.get("falsifications", 0) >= 0 and trad.get("refinements_reframes", 0) >= 0
    gate_f = len(candidates.get("candidates", [])) >= 1
    ...
    return {
        ...
        "gate_B_reframing": {
            ...
            "evidence": {
                ...
                "note": "Unexecuted frames do not pass Gate B.",
            },
        },
        "gate_C_evidence_driven_transition": {
            "result": "PASS" if gate_c else "FAIL",
            "evidence": f"{len([t for t in transitions if t.get('execution', {}).get('was_researched')])} executed transitions",
        },
        "gate_D_temporal_legality": {"result": "PASS" if gate_d else "FAIL", "evidence": temporal_audit},
        "gate_E_scientific_discipline": {"result": "PASS" if gate_e else "FAIL", "evidence": trad},
        "gate_F_discovery": {
            "result": "PASS" if gate_f else "FAIL",
            "evidence": f"{len(candidates.get('candidates', []))} conditional candidates",
            "note": "Gate F may FAIL while A-E PASS without architectural failure.",
        },
    }
...
def build_architectural_comparison(
    bb03_coverage: Dict[str, Any],
    bb03_scorecard: Dict[str, Any],
    result: Any,
) -> Dict[str, Any]:
    ...
    graph = result.graph
    trad = bb03_coverage.get("traditional_coverage", {})
    fc = bb03_coverage.get("frame_coverage", {})

    def row(metric: str, bb1: Any, bb2: Any, bb3: Any) -> Dict[str, Any]:
        return {"metric": metric, "bb01": bb1, "bb02": bb2, "bb03": bb3}

    table = [
        row("experiments", bb01_cov.get("total_experiments_executed", 2), bb02_cov.get("total_experiments_executed"), trad.get("experiments_executed")),
        row("pct_budget_used", bb01_cov.get("pct_budget_used"), bb02_cov.get("pct_budget_used"), trad.get("pct_budget_used")),
        row("features_touched", len(bb01_cov.get("features_touched", [])), len(bb02_cov.get("features_touched", [])), len(trad.get("features_touched", []))),
        row("tools", len(bb01_cov.get("tools_used", [])), len(bb02_cov.get("tools_used", [])), len(trad.get("tools_used", []))),
        row("research_frames_executed", 1, 1, fc.get("frames_executed")),
        row("outcome_specs_executed", len(bb01_cov.get("unique_outcome_specs", [])), len(bb02_cov.get("unique_outcome_specs", [])), fc.get("unique_outcome_specs_executed")),
        row("population_specs_executed", len(bb01_cov.get("unique_population_specs", [])), len(bb02_cov.get("unique_population_specs", [])), fc.get("unique_population_specs_executed")),
        row("observation_horizons_executed", 1, 1, len(fc.get("observation_horizons_executed", []))),
        row("frame_transitions", 0, 0, fc.get("frame_transitions_recorded")),
        row("outcome_reframes_executed", 0, 0, fc.get("outcome_reframes_executed")),
        row("population_reframes_executed", 0, 0, fc.get("population_reframes_executed")),
        row("outcome_to_population_executed", 0, 0, fc.get("outcome_to_population_executed")),
        row("falsifications", bb01_cov.get("falsification_experiments_executed"), bb02_cov.get("falsification_experiments_executed"), trad.get("falsifications")),
        row("conditional_candidates", 0, len(json.loads((BB02_ARTIFACTS / "04_candidate_discoveries.json").read_text())), len(collect_candidates(result)["candidates"])),
        row("terminal_status", bb01_summary.get("session_status"), bb02_summary.get("session_status"), graph.session.status.value),
        row("research_capability_score", bb01_score.get("overall_research_capability_score"), bb02_score.get("overall_research_capability_score"), bb03_scorecard.get("overall_research_capability_score")),
        row("reframing_capability_score", 0, 0, bb03_scorecard.get("reframing_capability_score")),
    ]
    ...
    return {
        ...
        "metrics_table": table,
        "bb02_limitation_addressed": "BB02: 1 OutcomeSpec + 1 PopulationSpec despite feature exploration",
        "bb03_primary_question": "Did Bot change the research frame (executed, not merely created)?",
    }
...
def write_partial_artifacts(
    ...
    """Generate best-effort BB03 artifacts from incomplete session."""
    ...
    scorecard = score_benchmark(diary, coverage, coverage, frame_transitions, result)
    gates = evaluate_capability_gates(coverage, coverage, frame_transitions, temporal_audit, candidates)
    ...
    (ARTIFACTS / "17_bb01_bb02_bb03_comparison.json").write_text(
    ...
    failure_report = {
        ...
        "architectural_finding": (
            "Autonomous OUTCOME_TO_POPULATION reframe was selected and executed, "
            "but session crashed during subsequent planning when grammar validation "
            "rejected horizon-advanced population fields in _add_grammar_candidates."
        ),
...
def main() -> None:
    ...
    print("=== BLIND BENCHMARK 03: load frozen panel (BB01/BB02 fingerprint) ===")
    ...
    print("=== Autonomous research session (budget=%d, Phase 3G.2) ===" % EXPERIMENT_BUDGET)
    ...
    if result is None:
        failure_report = {
            ...
            "interpretation": (
                ...
                "Research logic was NOT modified post-failure per BB03 policy."
            ),
        }
